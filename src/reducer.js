const reducer = (state, action) => {
    if (action.type === "TOGGLE_NOTE") {
                ...
                return tab.map((channel, channelIdx) => {
                    return channel.map((bar, barIdx) => {
                        return bar.map((beat, beatIdx) => {
                            if (channelIdx === action.payload.channel && barIdx === action.payload.bar && beatIdx === action.payload.beat) {
                                return !beat; 
                            } else {
                                return beat; 
                            }
        ...
        const newTabNotEmpty = newNoteStorage.map((tab) =>
            !tab.every((channel) => 
                channel.every((bar) =>
                    bar.every(beat => !beat)
                )
        ...
        return {
            ...
            channels: state.channels.map((channel, channelIdx) => {
                if (channelIdx === action.payload.channel)
                    return {...channel, instrument: parseInt(action.payload.instrument)}
                else 
                    return {...channel}
            })
        }
    ...
    if (action.type === "CHANGE_VOLUME") {
        ...
        return { ...state, channels: state.channels.map((channel, channelIdx) => {
